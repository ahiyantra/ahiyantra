...
class Menu:
  ...
  def __repr__(self):
    self.representative_string = "The \'" + str(self.name) + "\' menu is available from " + str(self.start_time) + " to " + str(self.end_time) + " everyday.";
    return self.representative_string;
  def calculate_bill(self, purchased_items):
    self.total=0;
    self.purchased_items=purchased_items;
    for v in self.purchased_items:
      self.total += self.items[v];
    print(self.total);
    return self.total;
...
class Franchise:
  def __init__(self, address, menus):
    ...
    self.menus=menus;
  def __repr__(self):
    self.representative_string = "This resturant is located at '" + str(self.address) + "'.";
    return self.representative_string;
  def available_menus(self, time):
    pr = [];
    self.time = time;
    for v in self.menus:
      if self.time >= v.start_time:
        if self.time <= v.end_time:
          pr.append(v.name);
    print(pr);
    return pr;
...
b0 = Business("Basta Fazoolin' with my Heart", [flagship_store, new_installment]);
...
b1 = Business("Take a' Arepa", [arepas_place]);
